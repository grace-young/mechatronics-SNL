...
#define PIN_BALL_COMMAND_SHOOT1       11
#define PIN_BALL_COMMAND_SHOOT2       13
...
void RespSlamLeft(){
    ...
    if(leftBumperHit()){//add a timer componenet to this in case of ball jamming occuring. millis() - fake_timer > TIME_TO_SLAM_LEFT 
      makeMotorsStop();
      armEye();
...
void RespHeadingBack(){
  if(backBumperHit()){//add some time component to this to make sure that balls that get stuck won't fuck us over 
    //millis()-fake_timer > TIME_TO_HEAD_BACK
...
bool ReadTapeSensor_F_2(void){
    // 1 if read tape, 0 if not 
    return digitalRead(PIN_SENSOR_F_2);
}
